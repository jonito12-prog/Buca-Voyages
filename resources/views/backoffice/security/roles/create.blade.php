@extends('layouts.app', ['title' => 'Créer un rôle - Buca Voyages VIP'])
    ...
    <main class="office-content narrow">
        <div class="page-head">
            <div>
                ...
                <h1>Créer un rôle</h1>
                <p>Définissez un profil utilisateur et ses privilèges applicatifs.</p>
            </div>
        </div>

        <form class="editor" method="POST" action="{{ route('security.roles.store') }}">
            @csrf
            
            <div class="form-grid">
                <div class="wide">
                    <label><span>Nom du rôle *</span>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Ex: Chef d'agence, Superviseur colis" required>
                    </label>
                </div>
                
                <div class="wide">
                    <label><span>Description</span>
                        <input type="text" name="description" value="{{ old('description') }}" placeholder="Ex: Dispose d'un accès complet aux opérations et aux validations financières.">
                    </label>
                </div>
                                ...
                                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 10px;">
                                    @foreach ($permissions as $permission)
                                        <label class="check" style="margin: 0; padding: 6px; background: white; border: 1px solid var(--line); border-radius: 6px; font-weight: 500; font-size: 13.5px; display: flex; align-items: center; gap: 8px; cursor: pointer; transition: all 0.2s;">
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="perm-checkbox-{{ Str::slug($module) }}" @checked(is_array(old('permissions')) && in_array($permission->id, old('permissions')))>
                                            <div>
            ...
            <div class="form-buttons" style="margin-top: 30px;">
                <button class="primary action-button" type="submit">Créer le rôle</button>
